...
from models.ESIM.layers import RNNDropout, Seq2SeqEncoder, SoftmaxAttention
import numpy as np
...
class LEAGUE(nn.Module):
    ...
    def forward(self, sentence,user,s_time):
        ...
        sen_lstmed, (final_hidden_state, final_cell_state)  = self.LSTM(sen_emd)  # conversation BILSTM层  (batch_size + 200,sen_len,embedding_dim(hidden) * 2)
        ...
        final_match = torch.mul(torch.mul(final_match,label_final),fun_time) # final_match (batch_Size,100,embedding_size * 2)
