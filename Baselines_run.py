    ...
    with torch.no_grad():
        if model_type == 'ESIM':
            for (q, q_len, h, h_len, label) in dataloader:
                ...
                q1_lengths = torch.where(q1_lengths == 0, 1, q1_lengths)
                q2_lengths = torch.where(q2_lengths == 0, 1, q2_lengths)
                labels = label.to(device)
                for count in range(q1.shape[0]):
                    if q1[count][0] == 0:
                        q1[count][0] = 1
                for count in range(q2.shape[0]):
                    if q2[count][0] == 0:
                        q2[count][0] = 1
...
def test(model, dataloader,model_type):
    """
...
def train(model, dataloader, optimizer, criterion, epoch_number, max_gradient_norm,model_type):
    """
    ...
    if model_type == 'ESIM':
        for batch_index, (q, q_len, h, h_len, label) in enumerate(tqdm_batch_iterator):
            ...
            all_prob.extend(probs[:, 1].cpu().detach().numpy())
            all_labels.extend(label)
    else:
