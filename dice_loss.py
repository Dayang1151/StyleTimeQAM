...
class DiceLoss(nn.Module):
    ...
    def _multiple_class(self, input, target, logits_size, mask=None):
        flat_input = input
        flat_target = F.one_hot(target.to(torch.int64), num_classes=logits_size).float() if self.index_label_position else target.float()
        ...
        if self.ohem_ratio > 0 :
            ...
            for label_idx in range(logits_size):
                ...
                if keep_num > 0:
                    ...
                    a = torch.argmax(flat_input, dim=1) == label_idx
                    b = flat_input[:, label_idx] >= threshold
                    c = pos_example.view(-1)
                    d = a & b
                    cond = d | c
